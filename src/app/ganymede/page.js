...
export default function Page() {
  ...
  const [currentIndex, setCurrentIndex] = React.useState(2); // Initial index
  const [direction, setDirection] = React.useState(""); // To track the direction of animation

  const handleLeftArrowClick = () => {
    setDirection("left");
    setCurrentIndex((prevIndex) => (prevIndex === 2 ? 5 : prevIndex - 1));
  };

  const handleRightArrowClick = () => {
    setDirection("right");
    setCurrentIndex((prevIndex) => (prevIndex === 5 ? 2 : prevIndex + 1));
  };
